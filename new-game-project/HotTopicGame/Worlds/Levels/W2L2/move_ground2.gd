...
func setup_connections():
	# Find the trigger
	moving_ground_trigger = get_node("../TriggerMove")  
	
	if moving_ground_trigger:
		
		# Connect to the correct signal
		if moving_ground_trigger.has_signal("triggered_moving_ground"):
			moving_ground_trigger.connect("triggered_moving_ground", start_moving)
			#print("MoveGround: Connected to trigger successfully")
		else:
			print("MoveGround: Signal 'triggered_moving_ground' not found on trigger")
	else:
